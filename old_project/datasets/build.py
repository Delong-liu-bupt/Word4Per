...
from .bases import ImageDataset, TextDataset, ImageTextDataset, ImageTextMLMDataset, GalleryDataset, QueryDataset

from .cuhkpedes import CUHKPEDES
from .icfgpedes import ICFGPEDES
from .rstpreid import RSTPReid
from .comreid import ComReid

__factory = {'CUHK-PEDES': CUHKPEDES, 'ICFG-PEDES': ICFGPEDES, 'RSTPReid': RSTPReid, 'ComReid': ComReid}
...
def build_dataloader(args, tranforms=None):
    logger = logging.getLogger("Word4Per.dataset")
...
def build_dataloader_toword(args, tranforms=None):
    ...
    ds = dataset.query
    test_query_set = QueryDataset(ds['instance_ids'], ds['img_paths'], ds['captions'],
                                test_transforms, text_length=args.text_length)
    
    ds = dataset.gallery
    test_gallery_set = GalleryDataset(ds['instance_ids'],
                                ds['img_paths'],
                                test_transforms)

    test_query_loader = DataLoader(test_query_set,
                                    batch_size=args.test_batch_size,
                                    shuffle=False,
                                    num_workers=num_workers)
    test_gallery_loader = DataLoader(test_gallery_set,
                                    batch_size=args.test_batch_size,
                                    shuffle=False,
                                    num_workers=num_workers)
    return test_query_loader, test_gallery_loader
